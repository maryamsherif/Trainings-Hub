...
public interface CourseRepository extends JpaRepository<Course, Integer> {
    List<Course> findAllByCategory(String category, Pageable paging);
    @Query("SELECT c FROM Comment c WHERE c.course.id = :courseId")
    List<Comment> findAllCommentsById(@Param("courseId") Integer courseId, Pageable paging);
    ...
    List<Course> searchCoursesByKeyword(@Param("keyword") String keyword, Pageable paging);
}